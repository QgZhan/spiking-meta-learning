...
class TieredImageNet(Dataset):
    ...
    def __init__(self, root_path, split='train', mini=False, **kwargs):
        split_tag = split
        data_path ='/home/wbc/projects/meta-baseline-slayer/materials/tiered-imagenet'     #################################################
        data = np.load(os.path.join(data_path, '{}_images.npz'.format(split_tag)),allow_pickle=True)['images']
        data = data[:, :, :, ::-1]
        with open(os.path.join( data_path, '{}_labels.pkl'.format(split_tag)), 'rb') as f:
